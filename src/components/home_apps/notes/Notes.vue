...
import FullNoteModal from "./FullNotesModal.vue";
...
<template>
  <div class="mx-auto max-w-screen-xl py-20 px-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
      <div v-for="note in notesStore.notes" :key="note.id"
        ...
        <div class="p-5">
          ...
          <p class="text-gray-800 text-sm break-words">{{ truncateContent(note.content, 100) }}</p>
        ...
        <div class="flex items-center justify-between text-gray-800 p-5">
          ...
          <div class="flex space-x-2">
            <button @click="openFullNoteModal(note)"
            ...
            <button @click="editNote(note)"
              class="w-8 h-8 rounded-full bg-gray-100 text-purple-800 flex items-center justify-center focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-violet-500"
              aria-label="edit note" role="button">
              <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-pencil" width="20" height="20"
    ...
    <NotesModal v-if="isNotesModalVisible" :note="currentNote" @close="closeNotesModal" @save="saveNote" />

    <FullNoteModal v-if="isFullNoteModalVisible" :content="fullNoteContent" @close="closeFullNoteModal" />
  </div>
</template>