...
<template>
    <div class="mx-auto max-w-screen-xl py-20 px-6">
      ...
      <div
        v-for="note in notes"
        :key="note.id"
        :class="`rounded ${note.color === 'pink' ? 'bg-pink-300' : 'bg-violet-300'} border border-gray-400 shadow-lg mb-6`"
        @click="note.content.length > 100 ? openFullNoteModal(note) : null" 
      >
          <div class="p-5">
